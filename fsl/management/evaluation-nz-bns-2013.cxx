...
using Fsl::Management::Procedure;
using Fsl::Management::ProcedureAny;

#include <fsl/management/evaluation.hpp>
using Fsl::Management::Evaluation;

#include <fsl/population/models/model.hpp>
using Fsl::Population::Models::Numbers;
...
public:
    ...
    public:
        ...
        public:
            ...
            // Need to assume a split between line and trawl for going forward
            double split;
            
            Fishing& initialise(void){
                split = 1700.0/2000.0;
            }
            ...
            template<
                ...
                class Management
            >
            Fishing& update(const Date& date, Fish& fish, const Management& management){
                line.;
                line.update(fish)
            ...
            double catches(void) const {
                return line.catches() + trawl.catches();
            }
        ...
        public:
            template<
                ...
                class Fishing
            >
            Monitoring& update(const Date& date,const Fish& fish, const Fishing& fishing){
                return *this;
            }
            
        } monitoring;

        ProcedureAny management;
        
        void update(const Date& date){
            if(date==Date(1936,10,1)){
                fish.initialise(10);
            } else {
                fish.update(date,fishing);
                fishing.update(date,fish,management);
                monitoring.update(date,fish,fishing);
                management.update(date,monitoring);
            }
        }
    ...
    public:
        ...
        class PriorMeanSetter{
        public:
            template<
                class Property,
                class Distribution
            >
            PriorMeanSetter& operator()(const Property& property,const Distribution& distibution){
                auto mean = distibution.mean();
                property.set(mean);
                return *this;
            }
        };
        ...
        void document(Model& model){
            ...
            for(Date date=conditioning.start;date<=conditioning.finish;date+=conditioning.delta){
                static_cast<Derived*>(this)->likelihood(likelihoods,date);
            }
            #endif
        }
        ...
        template<
            class Model
        >
        Conditioning& condition(Model& model){
            auto& self = *this;
            
            self.expected(model);
            
            auto date = self.start();
            auto finish = self.finish();
            do {
                model.update(date);
                self.log(date,model);
            } while (date<finish and self.step(date));
            return self;
        }
        
        Date start(void) const {
            return Date(1936,10,1);
        }
        
        Date finish(void) const {
            return Date(2012,10,1);
        }
        
        bool step(Date& date) const {
            date += Years(1);
            return true;
        }
        
        void log(const Date& date, const Model& model){
            std::cout<<date<<"\t"<<model.fish.biomass()<<"\t"<<model.fish.recruits()<<"\n";
        }
        ...
        template<
            class Model
        >
        Conditioning& expected(Model& model){
            auto& self = *this;
            PriorMeanSetter connector;
            self.priors(connector,model);
            return self;
        }
        ...
        template<
            ...
            class Model
        >
        void priors(Connector& connector,Model& model){
            ...
            connector
                ...
                // Proportion of males
                __(fish.split,Fixed(0.5))
                
                // S0 and R0 need to be set at something
                //! @todo
                __(fish.stock_recruit.relationship.s0,Fixed(10000))
                __(fish.stock_recruit.relationship.r0,Fixed(1000))
                
                // Steepness
                // Cordue & Pomarède (2012) : base case 0.75; sensitivity 0.9
                __(fish.stock_recruit.relationship.steepness,Fixed(0.75))

                // Variation
                // Cordue & Pomarède (2012) : 0.6
                __(fish.stock_recruit.variation.sd,VariationBentley2012())
                
                // Autocorrelation
                __(fish.stock_recruit.autocorrelation.coefficient, AutocorrelationBentley2012())
    ...
    public:
        ...
        //! @brief Candidate management
        
        std::vector<ProcedureAny> candidates_vector;
        
        int candidates(void) const {
            return candidates_vector.size();
        }
        
        ProcedureAny& candidate(const int& index) {
            return candidates_vector[index];
        }
        ...
        //! @brief Evaluate all management procedure candidates against the model
        template<
            class Model
        >
        Evaluation& evaluate(Model& model){
            auto& self = *this;
        ...
        //! @brief Evaluate a single management procedure candidate against the model
        template<
            class Model
        >
        Evaluation& evaluate(Model& model,const int& index){
            auto& self = *this;
            
            // Set management to be the candidate management procedure
            model.management = candidate(index);

            // Step through dates until finish date
            auto date = self.start();
            do {
                model.update(date);
                self.log(index,date,model);
            } while (self.step(date));
        ...
        void initialise(){
            ProcedureAny proc;
            proc = new Procedure;
            //! @todo pull up with a range of different parameters 
            candidates_vector.push_back(proc);
        }
        
        Date start(void) const {
            return Date(2013,10,1);
        }
        
        Date finish(void) const {
            return Date(2063,10,1);
        }
        ...
        void log(const int& index, const Date& date, const Model& model){
            std::cout<<index<<"\t"<<date<<"\t"<<model.fish.biomass()<<"\t"<<model.fish.recruits()<<"\t"<<model.fishing.catches()<<"\n";
        }
